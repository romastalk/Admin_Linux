...
for pc, detail in pc_dict.items():
    selection_menu_wake.append_item(
        cm.items.CommandItem(
            f"PC #{pc} IP: {detail['ip']} MAC: {detail['mac']}",
            f"wakeonlan {detail['mac']}",
        )
    )
    selection_menu_ping.append_item(
        cm.items.CommandItem(
            f"PC #{pc} IP: {detail['ip']} MAC: {detail['mac']}",
            f"ping -c 4 {detail['ip']}",
        )
    )
    # TODO теперь тут рут - работать будет только с прописанными ssh ключами
    selection_menu_exec_run.append_item(
        cm.items.FunctionItem(
            f"PC #{pc} IP: {detail['ip']} MAC: {detail['mac']}",
            execute_script,
            args=[detail["ip"], "root"],
        )
